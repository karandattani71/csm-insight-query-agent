...
class CSMAgent {
  constructor() {
    ...
    this.DB_NAME = "csm_vector_db";
    this.COLLECTION_NAME = "analytics_reports";
    this.TICKETS_COLLECTION = "support_tickets";
    this.EMBEDDING_MODEL = "Xenova/all-MiniLM-L6-v2";
    this.client = null;
  ...
  async searchInternal(query, k = 5) {
    ...
    return findSimilarDocuments(collection, queryVector, k, 0.5); // Use similarity threshold
  }
  ...
  //     questions.add(
  //       `Which region are you interested in? Available options: ${[...reportAnalysis.regions].join(", ")}`
  //     );
  //   }

  //   if (!queryAnalysis.timeframe && reportAnalysis.timeframes.size > 0) {
  //     questions.add(
  //       `What time period should we focus on? Available options: ${[...reportAnalysis.timeframes].join(", ")}`
  //     );
  //   }

  //   if (!queryAnalysis.product && reportAnalysis.products.size > 0) {
  //     questions.add(
  //       `Which product are you analyzing? Available options: ${[...reportAnalysis.products].join(", ")}`
  //     );
  //   }

  //   if (reports.length > 0 && reports[0].similarity < 0.6) {
  //     questions.add("Would you like to expand your search to related categories?");
  //   }

  //   // Fallback questions if no specific context is found
  //   if (questions.size === 0) {
  //     questions.add("Could you provide more details about your specific use case?");
  ...
  //     timeframe: timeframes.find((t) => query.includes(t)),
  ...
  analyzeQuery(query) {
    ...
    return {
      region: regions.find((r) => query.includes(r)),
      timeframe: timeframeMatch, // Use the matched timeframe
      product: products.find((p) => query.includes(p)),
      metrics: this.detectMetrics(query),
    };
  }
  ...
  notifyCSMTeam(ticket) {
    // Integration stub for team notification
  ...
  //       const answer = await this.getUserFeedback(rl, "Are these reports helpful? (y/n): ");
  //       if (answer === "y") {
  ...
  //     const questions = this.generateProbingQuestions(refinedQuery, internalReports);
  //     if (questions.length === 0) break; // No more probing questions

  //     const answer = await this.getUserFeedback(rl, `Probing: ${questions[0]} `);
  //     refinedQuery += ` ${answer}`;
  //     probingAttempts++;
  ...
  //       const answer = await this.getUserFeedback(rl, "Are these external reports helpful? (y/n): ");
  //       if (answer === "y") {
  //         console.log("Process completed. Exiting...");
  //         rl.close(); // Close the readline interface
  //         return;
  ...
  //   };
  //   await this.logSupportTicket(refinedQuery, [...internalReports, ...this.externalApiSearch(refinedQuery)], context);
